...
export default function EntryIntro(){
  ...
  const enter=()=>{if(exiting)return;setExiting(true);timer.current=setTimeout(()=>setVisible(false),760)};
  ...
  return <div className={"entry-intro "+(exiting?"is-exiting":"")}>
    ...
    <button className="entry-core" onClick={enter} aria-label="Enter Amrit Bhardwaj portfolio">
